...
/*
	while (p <= B2) {
		...
		scalar(mP, P, p, d, window_size, N);
		mpz_mul_mod(product, product, mP->X,N);
  //gmp_printf("P: ( %Zd , %Zd , %Zd )\n",P->X,P->Y,P->Z);
	...
	mpz_clear(product);
*/
	BSGS2(f,P,B1,B2,d,window_size,N,fp);
	gmp_printf("stage 2: factor=%Zd\n",f);
	end = omp_get_wtime();
